...
################################################################################################################
################################################################################################################
# first, we need to make the solution run as long as we don't have the whole room mapped out


# necessary data structures for this challenge

# need to add a function to go in the opposite direction of what we want
directions = ('n', 's', 'e', 'w')
...
# use a DFS have to pull this off
        # if you do a bfs and cannot find a room with an unexplored exit
        # you are done!

def append_unknown_to_graph(player, graph):
    graph[player.currentRoom.id] = {}
    for exit in player.currentRoom.getExits():
        graph[player.currentRoom.id][exit] = '?'

def add_unexplored_rooms(player, graph):
    pass
...
def traverseMap(player, direction=''):
    ...
    for exit in exits:
        if knownGraph[current_room_id][exit] == '?':
            next_direction == exit
            player.travel(exit)
            # lets change our current room to the room we are now in
            new_current_room = player.currentRoom.id
            prev_room = current_room_id
            knownGraph[prev_room][exit] = new_current_room
    ...
    if next_direction is '?':
        ...
        while queue.size() > 0:
            ...
            if current not in visited:
                ...
                if '?' in knownGraph[current].values():
                    # Return path to that room and reset the queue
                    travel_path = queue_in_check
                    queue = Queue()
                    break

                for neighbor in knownGraph[current].values():
                    new_path = queue_in_check[:]
                    new_path.append(neighbor)
                    queue.enqueue(new_path)

    for queued_room in travel_path:
        # for every room in the travel path
        room = player.currentRoom.id
        other_rooms = knownGraph[room].keys()
        for direction in other_rooms:
            if knownGraph[room][direction] == queued_room:
